...
mongodb.MongoClient.connect(DB_ADDRESS, { useNewUrlParser: true }, (err, client) => {
  if (err) {
    console.log(err);
    process.exit(1);
  }
...
app.post('/v1/collect', (req, res) => {
  ...
  db.collection(WEATHER_COLLECTION).insertOne(data, (err, doc) => {
    if (err) {
...
app.get('/v1/weather', (req, res) => {
  ...
  db.collection('weather').find({}).sort({ createdAt: -1 }).limit(limit).toArray((err, docs) => {
    if (err) {
...
app.get('/v1/ten-min-average', (req, res) => {
...
app.get('/v1/daily-highs', (req, res) => {
...
app.get('/v1/weekly-highs', (req, res) => {
...
app.get('/v1/weekly-lows', (req, res) => {
  handleAggregate(req.query, db, (timeAgo) => timeAgo.setDate(timeAgo.getDate() - 7), {
    _id: null,
    lowTemp: { $min: '$temp' },
    lowPressure: { $min: '$pressure' },
...
app.get('/v1/hourly-average', (req, res) => {
...
app.get('/v1/daily-average', (req, res) => {
...
app.get('/v1/issues', (req, expressRes) => {
